...
export default function TerminalBar({ className }) {
	...
	const extraButtons = (
		<div className="flex flex-col gap-2">
			...
			<div className="flex gap-2">
				...
				/>
				<label>Random Theme</label>
			</div>
			<hr />
			<div className="flex gap-2">
				<input type="checkBox" checked={wMMode} onChange={(event) => setWMMode((pre) => !pre)} />
				<label>WM mode</label>
			</div>
			{wMMode && (
				<>
	...
	return (
		<div
			className={`col-span-13 row-span-1 flex items-center justify-between gap-2 border-b-1 border-gray-600 bg-neutral-900 p-2`}
			style={{ gridArea: 'sitebar' }}
		>
			<span className="content-center px-2 text-center font-mono text-lg font-bold text-nowrap uppercase">
				terminal ugly
			</span>
			{/*
			...
			*/}
			<Popover.Root>
				<Popover.Trigger asChild onFocus={(event) => event.preventDefault()}>
					<button className="flex items-center gap-2 text-lg" type="button">
						<CgOptions className="inline bg-cyan-400 text-xl text-black" />
						<span>Options</span>
					</button>
				</Popover.Trigger>
				<Popover.Portal>
					...
					>
						<div className="flex flex-col gap-3 border border-gray-600 bg-neutral-900 p-4 text-indigo-200">
							{isWide ? (
								extraButtons
							) : (
								<>
			...
			<button
				...
				type="button"
			>
				<CiPalette className="inline bg-cyan-400 text-xl text-black" />
				<span>View Palate</span>
			</button>
